...
/**
	\file svm_optimizer_classification_unbiased_sbp.hpp
...
#ifndef __SVM_OPTIMIZER_CLASSIFICATION_UNBIASED_SBP_HPP__
#define __SVM_OPTIMIZER_CLASSIFICATION_UNBIASED_SBP_HPP__
	...
	m_totalAverageLoss( std::numeric_limits< double >::quiet_NaN() )
{
	...
	std::cout << "I am Unbiased SBP" << std::endl;

	if ( optimizerParameters.size() != 1 ) {

		std::stringstream stream;
		stream << "Unbiased::SBP optimizer expects one parameter (nu), received [";
	...
	if ( m_nu < 0 )
		throw std::runtime_error( "Unbiased::SBP optimizer parameter nu must be nonnegative" );
...
double const SBP::Objective() const {
	...
	return( m_totalKappa / m_iterations );
}
...
double const SBP::EvaluateHelper( t_VectorType const& vector ) const {
	...
	return( m_pKernel->Evaluate( vector, m_totalAlphas.Get() ) / m_totalKappa );
}
...
#endif    /* __SVM_OPTIMIZER_CLASSIFICATION_UNBIASED_SBP_HPP__ */
