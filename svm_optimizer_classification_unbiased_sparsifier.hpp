...
/**
	\file svm_optimizer_classification_unbiased_sparsifier.hpp
...
#ifndef __SVM_OPTIMIZER_CLASSIFICATION_UNBIASED_SPARSIFIER_HPP__
#define __SVM_OPTIMIZER_CLASSIFICATION_UNBIASED_SPARSIFIER_HPP__
	...
	m_maximumNormSquared( 0 )
{
	...
	std::cout << "I am Unbiased Sparsifier" << std::endl;

	if ( ( optimizerParameters.size() < 2 ) || ( optimizerParameters.size() > 4 ) ) {

		std::stringstream stream;
		stream << "Unbiased::Sparsifier optimizer expects between two and four parameters (prediction_file,norm^2,eta=1,suboptimality=inf), received [";
	...
	{	std::vector< double > data;
		...
		if ( data.size() != trainingSize )
			throw std::runtime_error( "Unbiased::Sparsifier optimizer parameter prediction_file must contain a prediction for every training example" );
	...
	if ( m_targetNormSquared < 0 )
		throw std::runtime_error( "Unbiased::Sparsifier optimizer parameter norm^2 must be nonnegative" );
	if ( optimizerParameters.size() > 2 ) {

		m_eta = atof( optimizerParameters[ 2 ].c_str() );
		if ( m_eta <= 0 )
			throw std::runtime_error( "Unbiased::Sparsifier optimizer parameter eta must be positive" );
...
double const Sparsifier::EvaluateHelper( Random::Generator::LaggedFibonacci4<>& generator, t_VectorType const& vector ) const {
	...
	return( m_pKernel->Evaluate( vector, m_alphas.get() ) + distribution.Sample( generator ) - 0.5 );
...
#endif    /* __SVM_OPTIMIZER_CLASSIFICATION_UNBIASED_SPARSIFIER_HPP__ */
