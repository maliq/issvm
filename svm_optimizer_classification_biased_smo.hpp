...
/**
	\file svm_optimizer_classification_biased_smo.hpp
...
#ifndef __SVM_OPTIMIZER_CLASSIFICATION_BIASED_SMO_HPP__
#define __SVM_OPTIMIZER_CLASSIFICATION_BIASED_SMO_HPP__
	...
	m_alphaSum(    std::numeric_limits< double >::quiet_NaN() )
{
	...
	std::cout << "I am Biased SMO" << std::endl;

	if ( optimizerParameters.size() != 1 ) {

		std::stringstream stream;
		stream << "Biased::SMO optimizer expects one parameter (lambda), received [";
	...
	if ( m_lambda < 0 )
		throw std::runtime_error( "Biased::SMO optimizer parameter lambda must be nonnegative" );
...
double const SMO::EvaluateHelper( t_VectorType const& vector ) const {
	...
	return( m_pKernel->Evaluate( vector, m_alphas.get() ) + m_bias );
}
...
#endif    /* __SVM_OPTIMIZER_CLASSIFICATION_BIASED_SMO_HPP__ */
